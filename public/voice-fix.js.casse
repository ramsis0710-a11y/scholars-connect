...
(function() {
  ...
  window.speak = function(text, lang) {
    ...
    if (prefix === 'ar') {
      ...
      chosen = getNativeVoiceForLanguage(targetLang);
      if (!chosen) {
        chosen = voices.find(function(v) { return v.lang.indexOf(prefix) === 0; });
      }
    }
  ...
  // ============================================================
  // MODULE LECTURE VOCALE - VOIX NATIVE PAR LANGUE + 3 ETATS
  // ============================================================
  var currentSpeechBtn = null;
  ...
  function getNativeVoiceForLanguage(langCode) {
    ...
    // Priorite 1 : voix qui correspond exactement au code langue
    var exact = voices.find(function(v) { return v.lang === langCode; });
    if (exact) return exact;

    // Priorite 2 : voix qui commence par le prefixe de langue (fr, ar, en...)
    var prefixMatch = voices.filter(function(v) { return v.lang.indexOf(prefix) === 0; });

    if (prefixMatch.length > 0) {
      // Preferer les voix masculines connues
      var maleNames = {
      ...
      var maleRegex = maleNames[prefix];
      if (maleRegex) {
        var maleVoice = prefixMatch.find(function(v) { return maleRegex.test(v.name); });
  ...
  window.toggleSpeech = function(btn, text, langOverride) {
    ...
    // Si un autre bouton jouait, le stopper
    if (currentSpeechBtn && currentSpeechBtn !== btn) {
      updateSpeechBtn(currentSpeechBtn, 'stopped');
    }

    // Si c'est le meme bouton
    if (currentSpeechBtn === btn && window.speechSynthesis) {
      if (speechSynthesis.speaking && !speechSynthesis.paused) {
        speechSynthesis.pause();
        updateSpeechBtn(btn, 'paused');
        return 'paused';
    ...
    var clean = String(text)
      ...
      .replace(/->/g, ' vers ').replace(/=>/g, ' donne ')
      .replace(/=/g, ' egale ').replace(/\+/g, ' plus ')
      .replace(/(\d)\s*-\s*(\d)/g, 'console.log('[voice-fix.js] V2 charge moins $2')
      .replace(/(\d)\s*\*\s*(\d)/g, 'console.log('[voice-fix.js] V2 charge fois $2')
      .replace(/(\d)\s*\/\s*(\d)/g, 'console.log('[voice-fix.js] V2 charge divise par $2')
      .replace(/\u00B2/g, ' au carre ')
      .replace(/\u00B3/g, ' au cube ')
    ...
    // Voix native correspondant a la langue
    var nativeVoice = getNativeVoiceForLanguage(lang);
    if (nativeVoice) {
      u.voice = nativeVoice;
      console.log('[voice-fix] Voix native :', nativeVoice.name, '(', nativeVoice.lang, ')');
    } else {
      console.warn('[voice-fix] Aucune voix native pour', lang);
    }

    u.onstart = function() { updateSpeechBtn(btn, 'playing'); };
    u.onend   = function() { updateSpeechBtn(btn, 'stopped'); currentSpeechBtn = null; currentUtterance = null; };
    u.onerror = function(e) {
      console.warn('[voice-fix] Erreur lecture :', e.error);
      updateSpeechBtn(btn, 'stopped');
      currentSpeechBtn = null;
